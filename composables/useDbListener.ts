...
export const createDbListener = (db: Database, path: string, cb: (snap: any) => void) => {
  ...
  if (!map) { map = new Map(); registry.set(db, map); }
  const prev = map.get(path) || 0;
  map.set(path, prev + 1);
  if (prev + 1 > 1) {
    // warn in console to help locate duplicate registration at runtime
    // eslint-disable-next-line no-console
    console.warn(`[useDbListener] multiple listeners registered for path=${path} count=${prev + 1}`);
  }
  ...
  return () => {
    ...
    try {
      ...
      if (m) {
        ...
        if (v <= 0) m.delete(path); else m.set(path, v);
      }
    } catch (e) { /* ignore */ }
  };
};
